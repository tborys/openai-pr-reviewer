...
async function handleInteractiveMode(
  ...
  const githubAnalyzer = GitHubPRAnalyzer.fromInteractiveContext(githubToken);
  ...
  const isRequestingReview = reviewKeywords.some(keyword => 
  ...
  if (isRequestingReview && prContext) {
    ...
    const enhancedReviewer = new OpenAIReviewer(openaiApiKey, {
      model,
      maxTokens,
      temperature: 0.1,
